...
// Extract everything from <div class="crm-card print-container border-0 shadow-none">
// up until right before <?php // Fetch version history
$startStr = '<div class="crm-card print-container border-0 shadow-none">';
$endStr = '<?php';
// We need to find the specific <?php that says "// Fetch version history"
$endPos = strpos($view, "<?php\n// Fetch version history");
if ($endPos === false) {
    // try different newline
    $endPos = strpos($view, "<?php\r\n// Fetch version history");
}
if ($endPos === false) {
    // Just find "Fetch version history" and trace back to <?php
    $fetchPos = strpos($view, "// Fetch version history");
    $endPos = strrpos(substr($view, 0, $fetchPos), "<?php");
}

$posStart = strpos($view, $startStr);
$quoteBlock = substr($view, $posStart, $endPos - $posStart);
...
echo "Successfully fixed public_quote.php length was " . strlen($quoteBlock) . "\n";
