...
$stmtItems = db()->prepare("SELECT * FROM order_items WHERE order_id=? ORDER BY sort_order ASC");
$stmtItems->execute([$id]);
$existingItems = $stmtItems->fetchAll();

$pageTitle = 'Edit Order: ' . $o['order_number'];
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            ...
            $stmt = db()->prepare("UPDATE orders SET order_number=?, customer_id=?, status=?, delivery_date=?, subtotal=?, discount_type=?, discount_value=?, discount_amount=?, tax_amount=?, total=?, notes=?, terms=? WHERE id=?");
            $stmt->execute([$order_number, $customer_id, $status, $delivery_date?:null, $subtotal, $discount_type, $discount_value, $discount_amount, $tax_amount, $total, $notes, $terms, $id]);
            ...
            db()->commit();
            logActivity('orders','update',"Updated order: $order_number",$id);
            setFlash('success',"Order '$order_number' updated.");
            header('Location: '.BASE_URL.'/modules/orders/view.php?id='.$id);
            exit;
        } catch (Exception $e) {
            db()->rollBack();
...
<div class="topbar">
  ...
  <div class="topbar-title">Edit Order</div>
</div>
<div class="page-content">
<?= flashHtml() ?>
<?php if(isset($errors['general'])): ?><div class="alert alert-danger"><?= e($errors['general']) ?></div><?php endif; ?>
<div class="page-header">
  <div class="page-header-left"><h1>Edit Order</h1>
    <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="<?= BASE_URL ?>/modules/dashboard/index.php">Home</a></li><li class="breadcrumb-item"><a href="<?= BASE_URL ?>/modules/orders/index.php">Orders</a></li><li class="breadcrumb-item active">Edit</li></ol></nav>
  </div>
  ...
  <div class="row g-3">
      ...
      <div class="crm-form-section">
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <input type="text" name="order_number" class="form-control" value="<?= e($o['order_number']) ?>">
          </div>
          <div class="col-md-6">
            ...
            <select name="customer_id" class="form-select <?= isset($errors['customer_id'])?'is-invalid':'' ?>">
              ...
              <?php foreach($customers as $c): ?>
                <option value="<?= $c['id'] ?>" <?= $o['customer_id']==$c['id']?'selected':'' ?>><?= e($c['name']) ?> <?= $c['company']?"({$c['company']})":'' ?></option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="col-md-6">
            ...
            <input type="date" name="delivery_date" class="form-control" value="<?= e($o['delivery_date']) ?>">
          </div>
          <div class="col-md-6">
      ...
      <div class="crm-form-section">
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <textarea name="notes" class="form-control" rows="4"><?= e($o['notes']) ?></textarea>
          </div>
          <div class="col-md-6">
            <label class="form-label">Terms & Conditions</label>
            <textarea name="terms" class="form-control" rows="4"><?= e($o['terms']) ?></textarea>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="col-lg-4">
      <div class="line-total-section mb-3">
        ...
        <div class="line-total-row-item align-items-center mb-2">
          ...
          <div class="input-group input-group-sm" style="width:140px">
              <input type="number" name="discount_value" id="discountValue" class="form-control text-end" value="<?= e($o['discount_value']) ?>" step="0.01" min="0" oninput="calculateTotal()">
      ...
      <div class="d-grid gap-2">
        ...
        <a href="<?= BASE_URL ?>/modules/orders/view.php?id=<?= $id ?>" class="btn btn-outline-secondary">Cancel</a>
      </div>
    </div>
  </div>
</form>
