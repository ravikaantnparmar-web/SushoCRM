...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        $imagePath = uploadFile($_FILES['image'], 'products', ALLOWED_IMAGES);
        if (!$imagePath && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) {
            $errors['image'] = 'Failed to upload image. Make sure it is a valid image file.';
        }
    }

    if (!$name)           $errors['name'] = 'Product name is required.';
    if ($selling_price < 0) $errors['selling_price'] = 'Price cannot be negative.';

    if ($sku) {
        $exists = db()->prepare("SELECT COUNT(*) FROM products WHERE sku=?");
        $exists->execute([$sku]);
        if ($exists->fetchColumn()) $errors['sku'] = 'SKU already exists.';
    }

    if (!$errors) {
        db()->prepare("INSERT INTO products (name, brand, sku, type, category_id, unit, package_qty, description, purchase_price, selling_price, tax_rate, stock_qty, min_stock, image, is_active, created_by)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
            ->execute([$name,$brand,$sku?:null,$type,$category_id,$unit,$package_qty,$description,$purchase_price,$selling_price,$tax_rate,$stock_qty,$min_stock,$imagePath,$is_active,$_SESSION['user_id']]);
        $id = db()->lastInsertId();
        logActivity('products','create',"Created product: $name",$id);
...
<div class="page-header">
  <div class="page-header-left"><h1>Add Product / Service</h1>
    <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="<?= BASE_URL ?>/modules/dashboard/index.php">Home</a></li><li class="breadcrumb-item"><a href="<?= BASE_URL ?>/modules/products/index.php">Products</a></li><li class="breadcrumb-item active">Add</li></ol></nav>
  </div>
      ...
      <div class="crm-form-section">
        ...
        <div class="row g-3">
          <div class="col-md-8">
            ...
            <input type="text" name="name" class="form-control <?= isset($errors['name'])?'is-invalid':'' ?>" value="<?= e($_POST['name']??'') ?>" placeholder="e.g. HP Laptop 15s">
          ...
          <div class="col-md-4">
            ...
            <select name="type" id="product_type" class="form-select">
              <option value="product" <?= ($_POST['type']??'product')==='product'?'selected':'' ?>>Product</option>
              <option value="service" <?= ($_POST['type']??'')==='service'?'selected':'' ?>>Service</option>
            </select>
          </div>
          <div class="col-md-6">
            <label class="form-label">SKU / Code</label>
            <input type="text" name="sku" class="form-control <?= isset($errors['sku'])?'is-invalid':'' ?>" value="<?= e($_POST['sku']??'') ?>" placeholder="Leave blank to auto-assign">
            <?php if(isset($errors['sku'])): ?><div class="invalid-feedback"><?= $errors['sku'] ?></div><?php endif; ?>
          </div>
          <div class="col-md-6">
            <label class="form-label">Brand</label>
            <input type="text" name="brand" class="form-control" value="<?= e($_POST['brand']??'') ?>" placeholder="e.g. Samsung">
          </div>
          <div class="col-md-6">
          ...
          <div class="col-md-4">
            ...
            <select name="unit" class="form-select">
              <?php foreach(['Nos','Pcs','Kg','Litre','Meter','Box','Set','Hour','Day','Month'] as $u): ?>
                <option value="<?= $u ?>" <?= ($_POST['unit']??'Nos')===$u?'selected':'' ?>><?= $u ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-4">
            <label class="form-label">Quantity per Unit</label>
            <input type="number" name="package_qty" class="form-control" value="<?= e($_POST['package_qty']??'1') ?>" step="0.01" min="0.01">
          </div>
          <div class="col-md-4">
            <label class="form-label">Tax Rate (%)</label>
            <select name="tax_rate" class="form-select">
              <?php foreach([0,5,12,18,28] as $t): ?>
                <option value="<?= $t ?>" <?= ($_POST['tax_rate']??18)==$t?'selected':'' ?>><?= $t ?>% <?= $t>0?'GST':'' ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="col-12">
            <label class="form-label">Description</label>
            <textarea name="description" class="form-control" rows="3" placeholder="Product description..."><?= e($_POST['description']??'') ?></textarea>
          </div>
          <div class="col-12">
    ...
    <div class="col-lg-4">
      <div class="crm-form-section">
        ...
        <div class="mb-3">
          ...
          <input type="number" id="purchase_price" name="purchase_price" class="form-control" value="<?= e($_POST['purchase_price']??'0') ?>" step="0.01" min="0">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
          ...
          <input type="number" id="selling_price" name="selling_price" class="form-control <?= isset($errors['selling_price'])?'is-invalid':'' ?>" value="<?= e($_POST['selling_price']??'0') ?>" step="0.01" min="0">
          <?php if(isset($errors['selling_price'])): ?><div class="invalid-feedback"><?= $errors['selling_price'] ?></div><?php endif; ?>
        </div>
      </div>
      <div class="crm-form-section" id="inventorySection">
        ...
        <div class="mb-3">
          ...
          <input type="number" name="stock_qty" class="form-control" value="<?= e($_POST['stock_qty']??'0') ?>" step="0.01" min="0">
        </div>
        <div class="mb-3">
          <label class="form-label">Minimum Stock Level</label>
          <input type="number" name="min_stock" class="form-control" value="<?= e($_POST['min_stock']??'0') ?>" step="0.01" min="0">
        </div>
        <div class="form-check mb-3">
          <input class="form-check-input" type="checkbox" name="is_active" id="isActive" value="1" <?= ($_POST['is_active']??1)?'checked':'' ?>>
          <label class="form-check-label" for="isActive">Active (visible in quotations/orders)</label>
        </div>
      </div>
      <div class="d-grid gap-2">
        <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i>Save Product</button>
        <a href="<?= BASE_URL ?>/modules/products/index.php" class="btn btn-outline-secondary">Cancel</a>
      </div>
    </div>
  </div>
</form>
...
function calculateSellingPrice() {
  ...
  const mp = parseFloat(markupPercent.value);
  if (!isNaN(mp) && pp > 0) {
    const sp = pp + (pp * mp / 100);
    sellingPrice.value = sp.toFixed(2);
  }
}

purchasePrice.addEventListener("input", calculateSellingPrice);
markupPercent.addEventListener("input", calculateSellingPrice);
