...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$purchase_number) $purchase_number = 'PO-' . date('Ymd') . '-' . rand(100,999);
    ...
    if (!$errors) {
        try {
            ...
            $stmt = db()->prepare("INSERT INTO purchases (purchase_number, vendor_id, status, purchase_date, subtotal, tax_amount, total, paid_amount, notes, terms, created_by)
                VALUES (?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->execute([$purchase_number, $vendor_id, $status, $purchase_date?:null, $subtotal, $tax_amount, $total, $amount_paid, $notes, $terms, $_SESSION['user_id']]);
            ...
            foreach ($items as $i => $item) {
                ...
                $stmtItem->execute([$purchase_id, $pid, $desc, $qty, $unit, $price, $tax_rate, $line_tax, $line_total]);

                if ($status === 'received' && $pid) {
            ...
            if ($new_balance != 0) {
                db()->prepare("UPDATE vendors SET outstanding_balance = outstanding_balance + ? WHERE id=?")->execute([$new_balance, $vendor_id]);
            }

            db()->commit();
            logActivity('purchases','create',"Created purchase order: $purchase_number",$purchase_id);
            setFlash('success',"Purchase '$purchase_number' created.");
            header('Location: '.BASE_URL.'/modules/purchases/view.php?id='.$purchase_id);
            exit;
        } catch (Exception $e) {
            db()->rollBack();
...
<div class="topbar">
  ...
  <div class="topbar-title">Create Purchase</div>
</div>
<div class="page-content">
<?= flashHtml() ?>
<?php if(isset($errors['general'])): ?><div class="alert alert-danger"><?= e($errors['general']) ?></div><?php endif; ?>
<div class="page-header">
  <div class="page-header-left"><h1>Create Purchase</h1>
    <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="<?= BASE_URL ?>/modules/dashboard/index.php">Home</a></li><li class="breadcrumb-item"><a href="<?= BASE_URL ?>/modules/purchases/index.php">Purchases</a></li><li class="breadcrumb-item active">Create</li></ol></nav>
  </div>
  ...
  <div class="row g-3">
      ...
      <div class="crm-form-section">
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <input type="text" name="purchase_number" class="form-control" value="<?= e($_POST['purchase_number']??'') ?>" placeholder="Auto-generated if blank">
          </div>
          <div class="col-md-6">
            ...
            <select name="vendor_id" class="form-select <?= isset($errors['vendor_id'])?'is-invalid':'' ?>">
              ...
              <?php foreach($vendors as $v): ?>
                <option value="<?= $v['id'] ?>" <?= ($_POST['vendor_id']??'')==$v['id']?'selected':'' ?>><?= e($v['name']) ?> <?= $v['company']?"({$v['company']})":'' ?></option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="col-md-6">
            ...
            <input type="date" name="purchase_date" class="form-control" value="<?= e($_POST['purchase_date']??date('Y-m-d')) ?>">
          </div>
          <div class="col-md-6">
            <label class="form-label">Status</label>
            <select name="status" class="form-select">
              <option value="pending" <?= ($_POST['status']??'pending')==='pending'?'selected':'' ?>>Pending</option>
              <option value="received" <?= ($_POST['status']??'')==='received'?'selected':'' ?>>Received</option>
            </select>
            <div class="form-text small">Marking as Received will automatically update inventory stock.</div>
          </div>
        </div>
      </div>
      ...
      <div class="crm-form-section">
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <textarea name="terms" class="form-control" rows="3"><?= e($_POST['terms'] ?? "1. Please supply items as per specifications.\n2. Payment will be processed within 30 days of delivery.") ?></textarea>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="col-lg-4">
      ...
      <div class="d-grid gap-2">
        ...
        <a href="<?= BASE_URL ?>/modules/purchases/index.php" class="btn btn-outline-secondary">Cancel</a>
      </div>
    </div>
  </div>
</form>
    ...
    tr.innerHTML = `
        <td>
            <select name="items[${itemIndex}][product_id]" class="form-select form-select-sm mb-1 product-select" onchange="productSelected(this, ${itemIndex})">
                ...
                ${products.map(p => `<option value="${p.id}" data-price="${p.purchase_price}" data-unit="${p.unit}" data-tax="${p.tax_rate}">${p.name}</option>`).join('')}
            </select>
