...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            ...
            $stmt = db()->prepare("INSERT INTO quotations (quote_number, customer_id, project_name, contact_person, status, valid_until, subtotal, discount_type, discount_value, discount_amount, tax_amount, total, notes, terms, created_by)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->execute([$quote_number, $customer_id, $project_name, $contact_person, $status, $valid_until?:null, $subtotal, $discount_type, $discount_value, $discount_amount, $tax_amount, $total, $notes, $terms, $_SESSION['user_id']]);
            ...
            foreach ($admins as $admin) {
                createNotification($admin['id'], 'Approval Required', "Quotation {$quote_number} requires your approval.", 'warning', '/modules/quotations/view.php?id='.$quote_id);
            }
