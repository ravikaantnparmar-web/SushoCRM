...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$project_number) $project_number = 'PRJ-' . date('Ym') . '-' . rand(100,999);
    ...
    if (!$errors) {
        $stmt = db()->prepare("
            INSERT INTO projects (
                ...
                start_date, target_end_date, expected_duration, completion_percentage, 
        ...
        $stmt->execute([
            ...
            $start_date?:null, $target_end_date?:null, $expected_duration?:null, $completion_percentage,
            $budget, $project_cost, $manager_id,
        ...
        logActivity('projects','create',"Created project: $project_number", $project_id);
        setFlash('success',"Project '$name' created successfully.");
        header('Location: '.BASE_URL.'/modules/projects/view.php?id='.$project_id);
        exit;
    }
}
...
<div class="topbar">
  ...
  <div class="topbar-title">Create Project</div>
</div>
<div class="page-content">
<div class="page-header">
  <div class="page-header-left"><h1>Create Project</h1>
    <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="<?= BASE_URL ?>/modules/dashboard/index.php">Home</a></li><li class="breadcrumb-item"><a href="<?= BASE_URL ?>/modules/projects/index.php">Projects</a></li><li class="breadcrumb-item active">Create</li></ol></nav>
  </div>
      ...
      <div class="crm-card-body p-4">
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <input type="text" name="project_number" class="form-control" value="<?= e($_POST['project_number']??'') ?>" placeholder="Auto-generated if blank">
            </div>
            ...
            <div class="col-md-6">
              ...
              <select name="project_type" class="form-select">
                ...
                <?php foreach(['Residential','Commercial','Hospitality','Retail','Villa','Apartment','Office'] as $opt): ?>
                  <option value="<?= $opt ?>" <?= ($_POST['project_type']??'')===$opt?'selected':'' ?>><?= $opt ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="col-md-6">
              ...
              <select name="project_category" class="form-select">
                ...
                <?php foreach(['Interior','Construction','Renovation','Turnkey','Architecture'] as $opt): ?>
                  <option value="<?= $opt ?>" <?= ($_POST['project_category']??'')===$opt?'selected':'' ?>><?= $opt ?></option>
                <?php endforeach; ?>
              </select>
            </div>
      ...
      <div class="crm-card-body p-4">
          <div class="row g-3">
            <div class="col-12">
              ...
              <textarea name="site_address" class="form-control" rows="2"><?= e($_POST['site_address']??'') ?></textarea>
            </div>
            <div class="col-md-4">
              <label class="form-label">City</label>
              <input type="text" name="site_city" class="form-control" value="<?= e($_POST['site_city']??'') ?>">
            </div>
            <div class="col-md-4">
              <label class="form-label">State</label>
              <input type="text" name="site_state" class="form-control" value="<?= e($_POST['site_state']??'') ?>">
            </div>
            <div class="col-md-4">
              <label class="form-label">Pincode</label>
              <input type="text" name="site_pincode" class="form-control" value="<?= e($_POST['site_pincode']??'') ?>">
            </div>
            <div class="col-12">
              <label class="form-label">Google Maps Location Link</label>
              <input type="url" name="google_maps_location" id="google_maps_location_input" class="form-control" value="<?= e($_POST['google_maps_location']??'') ?>" placeholder="https://goo.gl/maps/...">
            ...
            <div class="col-md-4">
              ...
              <input type="text" name="site_contact_person" class="form-control" value="<?= e($_POST['site_contact_person']??'') ?>">
            </div>
            <div class="col-md-4">
              <label class="form-label">Site Contact Number</label>
              <input type="text" name="site_contact_number" class="form-control" value="<?= e($_POST['site_contact_number']??'') ?>">
            </div>
            <div class="col-md-4">
              <label class="form-label">Site Engineer (Name/Number)</label>
              <input type="text" name="site_engineer_name_number" class="form-control" value="<?= e($_POST['site_engineer_name_number']??'') ?>">
            </div>
          </div>
      </div>
    </div>
          ...
          <div class="row g-3">
            ...
            <div class="col-12">
              ...
              <select name="stage" class="form-select">
                <option value="planning" <?= ($_POST['stage']??'planning')==='planning'?'selected':'' ?>>Planning</option>
                <option value="design" <?= ($_POST['stage']??'')==='design'?'selected':'' ?>>Design</option>
                <option value="execution" <?= ($_POST['stage']??'')==='execution'?'selected':'' ?>>Execution</option>
                <option value="finishing" <?= ($_POST['stage']??'')==='finishing'?'selected':'' ?>>Finishing</option>
                <option value="handover" <?= ($_POST['stage']??'')==='handover'?'selected':'' ?>>Handover</option>
                <option value="completed" <?= ($_POST['stage']??'')==='completed'?'selected':'' ?>>Completed</option>
              </select>
            </div>
            <div class="col-12">
              ...
              <select name="manager_id" class="form-select">
                ...
                <?php foreach($users as $u): ?>
                  <option value="<?= $u['id'] ?>" <?= ($_POST['manager_id']??'')==$u['id']?'selected':'' ?>><?= e($u['name']) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="col-12">
            ...
            <div class="col-12">
              ...
              <input type="date" name="target_end_date" class="form-control" value="<?= e($_POST['target_end_date']??'') ?>">
            </div>
            <div class="col-12">
              <label class="form-label">Expected Duration</label>
              <input type="text" name="expected_duration" class="form-control" value="<?= e($_POST['expected_duration']??'') ?>" placeholder="e.g. 6 Months">
            </div>
            <div class="col-12">
              <label class="form-label">Completion Percentage</label>
              <div class="input-group">
                <input type="number" name="completion_percentage" class="form-control" value="<?= e($_POST['completion_percentage']??'0') ?>" min="0" max="100">
  ...
  <div class="col-12">
    <div class="crm-card">
        <div class="crm-card-body p-4 d-flex gap-2">
            ...
            <a href="<?= BASE_URL ?>/modules/projects/index.php" class="btn btn-outline-secondary">Cancel</a>
        </div>
    </div>
  </div>
</div>
...
<?php /* Quick Customer Add Modal and JS are below */ ?>
...
(function() {
    ...
    saveBtn.addEventListener('click', function() {
        ...
        .then(data => {
            if (data.success) {
                // Refresh CSRF token for next use
